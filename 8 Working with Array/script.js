...
btnTransfer.addEventListener('click', function (e) {
  ...
  // to clear the transfer input fields
...
checkDogs([9, 16, 6, 8, 3], [9, 16, 6, 8, 3]);*/
...
const movementsUSDfor = []; //another method to do it.
...
//we can further write the code above like dis
...
//   return acc + cur;
// }, 0); //changing it to arrow function
...
for (const acc of accounts) {
  const user = [];
  if (acc.owner === 'Jessica Davis') user.push(acc);
  console.log(user);
}*/
...
labelBalance.addEventListener('click', function () {
  ...
  // console.log(movementUI.map(el => el.textContent.replace('€', ''))); we can do it this way below
...
const checkDogs = function (juliaDogs, kateDogs) {
  const juliaCorrected = juliaDogs.slice(1, -2);
  const newArr = juliaCorrected.concat(kateDogs);
  console.log(newArr);
